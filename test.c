...
void regija(int x,int y,char boja,char pamti)
    {
        if (tablica[x][y]==boja) return;
        tablica[x][y]=boja;
        if((x-1)>=0&&(x-1)<N&&y>=0&&y<M&&(tablica[x-1][y]==pamti)) regija(x-1,y,boja,pamti);
        if((x+1)>=0&&(x+1)<N&&y>=0&&y<M&&(tablica[x+1][y]==pamti)) regija(x+1,y,boja,pamti);
        if(x>=0&&x<N&&(y-1)>=0&&(y-1)<M&&(tablica[x][y-1]==pamti)) regija(x,y-1,boja,pamti);
        if(x>=0&&x<N&&(y+1)>=0&&(y+1)<M&&(tablica[x][y+1]==pamti)) regija(x,y+1,boja,pamti);
    }
int main()
{
    ...
    switch(c)
    {
        case 'I':
            {
                scanf("%d %d",&M,&N);
                if(N>250 ||  M>250) printf("N i M moraju biti manji ili jednaki 250");
